...
export default function PaymentPage({ onConfirm }: Props) {
  ...
  // Handler submit saat klik tombol Confirm
  const handleConfirmPayment = () => {
    ...
    // if parent provided onConfirm, call it so controller can show success
    if (onConfirm) {
      onConfirm();
      return;
    }
                ...
                >
                  <Upload
                    ...
                    className={`mb-4 transition-colors ${
                      isDragging ? "text-[#C58A1C]" : "text-white/70"
                    }`}
                  />
                  ...
                  <span className="mt-3 font-raleway text-lg text-white/50">
                    {selectedFile ? selectedFile.name : "1 file, PNG / JPG, max 4 mb"}
                  </span>

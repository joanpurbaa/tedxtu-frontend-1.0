...
export default async function TicketPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!ticket) {
    return <main className="min-h-screen bg-black text-white flex items-center justify-center">Ticket not found</main>;
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-8 gap-4">
      ...
      <div className="bg-white p-4 rounded-2xl">
        <Image src={`/api/qr/${ticket.qrToken}`} alt="QR Ticket" width={300} height={300} />
      </div>
      <p className="text-sm text-white/50">
        {ticket.scanned ? "Already scanned at venue" : "Show this QR at the entrance"}
      </p>
    </main>
  );
}

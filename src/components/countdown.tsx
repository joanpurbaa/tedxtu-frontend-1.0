...
export default function Countdown({
    ...
    const { totalHours, minutes, seconds, isFinished } = useCountdown(targetDate);

    return (
        <section
            aria-label='Event countdown'
            className={cn(
                'fixed left-0 right-0 top-0 z-50 flex h-12 w-full items-center justify-center overflow-hidden bg-[url(/countdown-background.png)] bg-cover bg-center bg-no-repeat px-4 text-center shadow-[0_10px_24px_rgba(0,0,0,0.65)] sm:h-[77px]',
                className
            )}
        >
            ...
            <p className='relative z-10 font-raleway text-xs font-black uppercase tracking-[0.12em] text-white drop-shadow-[0_2px_3px_rgba(0,0,0,0.85)] sm:text-base md:text-lg'>
                {isFinished ? (
                    ...
                    <>
                        ...
                        >
                            {totalHours}H {formatTime(minutes)}M {formatTime(seconds)}S
                        </time>
                    </>
                )}
            </p>
